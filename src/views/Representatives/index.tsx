...
const RepresentativesView = ({ navigation, route }: HPro.Props<HPro.SalesStack, 'RepresentativeView'>) => {
	...
	const [list, setList] = useState<HPro.representativeType[]>([])
	const [filteredList, setFilteredList] = useState<HPro.representativeType[]>([])

	/* Hooks */
	useEffect(() => {
		const getInfos = () => {
			executeSqlAsync(`select * from hrep order by raz`)
				.then(({ rows }) => {
					setList(rows._array)
