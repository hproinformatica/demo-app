...
const ClientView = ({ navigation, route }: HPro.Props<HPro.SalesStack, 'ClientView'>) => {
	...
	const [filteredList, setFilteredList] = useState<HPro.clientType[]>([])

	/* Hooks */
	useEffect(() => {
		const getInfos = () => {
			executeSqlAsync(`select * from hcli order by raz`)
				.then(({ rows }) => {
					setList(rows._array)
