...
const MaterialView = ({ navigation, route }: HPro.Props<HPro.SalesStack, 'MaterialView'>) => {
	...
	const [filteredList, setFilteredList] = useState<HPro.materialType[]>([])

	/* Hooks */
	useEffect(() => {
		const getInfos = () => {
			executeSqlAsync(`select * from hmat order by des`)
				.then(({ rows }) => {
					setList(rows._array)
